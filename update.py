...
def build_one(wiki, lang, fast):
    ...
    # Child-local env var, read by conf.py / common_conf hooks to set the
    # language switcher context and the cross-wiki menu prefix. Safe under
    # multiprocessing because each build_one runs in its own process.
    ...
    try:
        app = Sphinx(
            ...
            parallel=2,
            srcdir=source_dir,
            # html_search_language (zh word segmentation) is derived from
            # the language by common_conf's config-inited hook — one code
            # path for every builder (here, RTD, bare sphinx-build).
            confoverrides={'language': lang},
        )
        app.build()
    except Exception as exc:
        print(f"[update.py]: [ERROR]: Sphinx build exception for {wiki} [{lang}]: {exc}", file=sys.stderr)
        sys.exit(1)
...
def sphinx_make(site, parallel, fast, languages):
    ...
    for wiki, lang in jobs:
        p = multiprocessing.Process(name=f"build_one_{wiki}_{lang}",
                                    target=build_one, args=(wiki, lang, fast))
        p.start()
        procs.append(p)
    ...
    for wiki in ALL_WIKIS:
        ...
        for lang in languages:
            ...
            if not os.path.exists(index_html):
                fatal(f"{wiki} [{lang}] site not built - missing {index_html}")
...
def copy_build(site, destdir, languages):
    ...
    for wiki in ALL_WIKIS:
        ...
        for lang in languages:
            ...
            sourcedir = f'./{wiki}/build/html-{lang}/'
            try:
...
def make_backup(building_time, site, destdir, backupdestdir, languages):
    """
    ...
    for wiki in ALL_WIKIS:
        ...
        for lang in languages:
            prefix = URL_PREFIX.get(lang, lang)
            debug(f'Backing up: {wiki} [{prefix}]')
            ...
            bkdir = os.path.join(backupdestdir, f"{building_time}-wiki-bkp", prefix, str(wiki))
            ...
            try:
                ...
                error(ex)
                fatal(f"Failed to backup {wiki} [{prefix}]")
...
def copy_common_source_files(start_dir=COMMON_DIR, clean_common=False, site: Optional[str] = None):
    ...
    files_copied = 0
    files_skipped = 0
    # i18n manifest: vehicle -> docnames it received from common/. Consumed
    # by scripts/i18n_sync_common.py to keep translations deduplicated in
    # locale/common/.
            ...
            if file.endswith(".rst"):
                ...
                for wiki in targets:
                    if wiki not in allowed_wikis:
                        continue
                    # Record even when the copy below is skipped as unchanged:
                    # the manifest describes what belongs to the wiki, not
                    # what was rewritten this run.
    ...
    try:
        merged = {}
        if site is not None and os.path.exists(COMMON_MANIFEST_PATH):
            with open(COMMON_MANIFEST_PATH, 'r', encoding='utf-8') as f:
                merged = json.load(f)
        ...
        debug(f'wrote {COMMON_MANIFEST_PATH} ({sum(len(v) for v in merged.values())} entries)')
    except Exception as e:
        error(f'failed writing {COMMON_MANIFEST_PATH}: {e}')
...
class WikiUpdater:
    def __init__(self) -> None:
        ...
        parser.add_argument(
            ...
            help=("Comma-separated list of Sphinx language codes to build, "
                  "e.g. 'en,zh_CN'. Output goes to <destdir>/<url-prefix>/."),
        )
        parser.add_argument(
            "--fast",
            dest="fast",
    ...
    def run(self) -> None:
        ...
        languages = [c.strip() for c in self.args.languages.split(',') if c.strip()]
        ...
        info(f"Time elapsed so far: {time.time() - tstart:.2f} seconds")
        copy_static_html_sites(self.args.site, self.args.destdir, languages)
        ...
        info(f"Time elapsed so far: {time.time() - tstart:.2f} seconds")
        sphinx_make(self.args.site, self.args.parallel, self.args.fast, languages)
        if self.args.paramversioning:
            put_cached_parameters_files_in_sites(self.args.site, languages)
            cache_parameters_files(self.args.site, languages)

        check_build(self.args.site, languages)

        if self.args.enablebackups:
            make_backup(building_time, self.args.site, self.args.destdir, self.args.backupdestdir, languages)
            delete_old_wiki_backups(self.args.backupdestdir, N_BACKUPS_RETAIN)

        if self.args.destdir:
            copy_build(self.args.site, self.args.destdir, languages)
